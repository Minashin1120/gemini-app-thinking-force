...
- 初回インストール時に利用規約・プライバシーポリシーへの同意を必須化（同意するまで動作しない・更新時には再表示しない）
- 拡張アイコンのポップアップから状態確認
...
初回インストール時（または古いバージョンから更新して未同意の場合）は、利用規約・プライバシーポリシーの同意ページが自動で開きます。**同意するまで拡張機能は動作しません**。同意の確認は一度だけ表示され、同意後は更新でも再表示されません。
...
| 拡張アイコン | 接続状態・チップ表示・設定書き込み結果を表示 |
| 今すぐ有効化 | ポップアップから手動で再試行 |

状態確認には **gemini.google.com のタブが開いていること** が必要です。
...
window.__geminiThinkingAuto.dump()       // 状態を出力
...
│   ├── manifest.json
│   ├── background.js   # 初回インストール時の同意ページ表示
│   ├── page-hook.js    # MAIN world: fetch フック + UI 操作
│   ├── content.js      # isolated world: 同意状態の橋渡し + ポップアップ連携
│   ├── welcome.html    # 初回同意ページ（規約・ポリシー表示）
│   ├── terms.html      # 利用規約（拡張内表示用）
│   ├── privacy.html    # プライバシーポリシー（拡張内表示用）
│   ├── popup.*         # 拡張アイコンの UI
│   └── icons/
├── site/               # ウェブ公開用（Cloudflare 等）の規約・ポリシー単一 HTML
├── docs/               # HAR 調査メモなど
├── tools/              # 調査用スクリプト
└── README.md
```
...
- 動作確認用のログを収集・保存・表示しません（ポップアップにログ表示やダウンロード機能はありません）
- 拡張ポップアップや状態 API にセッション情報（`at` トークン等）の実値を含めません（存在有無のみ表示）
- 利用規約・プライバシーポリシーへの同意状態は、このブラウザの拡張機能専用ストレージにのみ保存されます（外部送信なし・更新後も維持）
